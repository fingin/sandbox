...
def public_key_to_private_key(public_key):  
    """
    Accept a public key and convert it to its respective private key.
    Average Time: 0.0000801390 seconds
    """
    return PrivateKey.fromString(bytes.fromhex(public_key)).toString().hex().upper()
...
def address_to_private_key(address):
    """
    Accept an address and convert it to its respective public key.
    Average Time: 0.0000801390 seconds
    """
    return PrivateKey.fromString(bytes.fromhex(address)).toString().hex().upper()
...
def address_to_public_key(address):
    ...
    Average Time: 0.0000801390 seconds
    """
    return PublicKey.fromString(bytes.fromhex(address)).toString().hex().upper()

if __name__ == '__main__':
    address = 'bc1qwqdg6squsna38e46795at95yu9atm8azzmyvckulcc7kytlcckxswvvzej'
    #TODO: make address to private key
    public_key = address_to_public_key(address)
    private_key = address_to_private_key(address)
